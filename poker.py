...
def evaluate_hand(player_hand, community_cards):
    ...
    # Check for straight
    is_straight = any(ranks[i:i+5] == sorted(rank_counts) for i in range(len(ranks)-4))
    ...
    if list(rank_counts.values()).count(2) == 2:
        ...
        return "Two Pair", max(pair_ranks), min(pair_ranks)
...
def main():
    ...
    while True:
        ...
        for player, hand in enumerate(players, start=1):
            ...
            print(f"Hand strength: {evaluate_hand(hand, flop + turn + river)}")
